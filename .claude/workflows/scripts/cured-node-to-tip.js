...
HARD RULES: consensus parity inviolable; every malloc via zcl_malloc, every error return LOG_*, every write via AR lifecycle; files < 800 lines (app/+config ENFORCED); every thread supervised; recovery-apply paths stay CONTAINED (respect check-no-new-borrowed-seed / check-no-new-repair-rung). If your worktree cannot link, FIRST: cp -a /home/rhett/github/zclassic23/vendor/lib vendor/. You are in your OWN isolated git worktree — commit to THIS worktree's branch only; do NOT push/merge/touch others. Self-gate: make build-only + focused test group (make t-fast ONLY=<group>) + make lint (+ agent_impact_rules.def per changed .c). Final message = structured data, technical; report the branch name and root_cause (real-defect vs test-artifact).`
