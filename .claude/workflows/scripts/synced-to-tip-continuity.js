...
HARD RULES: consensus parity inviolable; copy-prove before any live effect; recovery-apply paths stay CONTAINED (verify-only, refuse public/dev lanes, NEVER auto-run a borrowed-seed install — respect the check-no-new-borrowed-seed / check-no-new-repair-rung lint gates; any rung you add points the operator at the import + copy-prove, it does not autonomously mutate); every malloc via zcl_malloc, every error return LOG_*, every write via AR lifecycle; files under 800 lines; every thread supervised. If your worktree cannot link (missing vendor/lib/*.a), FIRST run: cp -a /home/rhett/github/zclassic23/vendor/lib vendor/. You are in your OWN isolated git worktree — commit to THIS worktree's branch only; do NOT push/merge/touch other worktrees. NEVER touch a live datadir, stop zclassicd, or disturb the mint producer. Self-gate: make build-only + focused test group (make t-fast ONLY=<group>) + make lint (+ agent_impact_rules.def mapping per changed .c; reconcile DOC-COUNTS / EXPECTED_DIAGNOSTICS_DUMPERS if you add a dumper). Final message = structured data, precise + technical; report the branch name.`
