...
HARD RULES: consensus parity inviolable (no new opcode; validate against the real chain, not zclassicd source text); all-or-nothing atomicity (any anomaly rolls back leaving cursors POSITIVE + blockers raised — a safe halt, never a partial flip); every malloc via zcl_malloc, every error return via LOG_*, every write via AR lifecycle; files under 800 lines (app/+config ENFORCED, lib WARN); NEVER touch a live datadir or stop zclassicd (you MAY read $HOME/.zclassic READ-ONLY). If your worktree cannot link (undefined vendor symbols / missing vendor/lib/*.a), FIRST run: cp -a /home/rhett/github/zclassic23/vendor/lib vendor/ (the static libs are gitignored and absent in a fresh worktree). You are in your OWN isolated git worktree — work only here. Self-gate: make build-only, the focused test group via make t-fast ONLY=<group>, and make lint (add agent_impact_rules.def mapping for every changed .c). Commit green work to THIS worktree's branch; do NOT push, do NOT merge to main, do NOT touch any other worktree. Your final message is structured data for the orchestrator — precise and technical; report the branch name you committed to.`
