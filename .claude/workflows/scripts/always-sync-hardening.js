...
HARD RULES: consensus parity inviolable; copy-prove before any live effect (owner-gated; recovery-apply paths are CONTAINED — they refuse on public/dev lanes and NEVER auto-run a borrowed-seed install; the lint gates check-no-new-borrowed-seed / check-no-new-repair-rung EXIST — respect them: any rung you add must be a contained/verify-only surface, not an autonomous live mutation); every malloc via zcl_malloc, every error return LOG_*, every write AR lifecycle; files under 800 lines; every thread supervised. If your worktree cannot link (undefined vendor symbols / missing vendor/lib/*.a — the static libs are gitignored and absent in a fresh worktree), run make worktree-init first. You are in your OWN isolated git worktree — work only here, commit to THIS worktree's branch, do NOT push/merge/touch other worktrees. Self-gate: make build-only + focused test group (make t-fast ONLY=<group>) + make lint (+ agent_impact_rules.def mapping per changed .c). Final message = structured data, precise + technical; report the branch name.`
