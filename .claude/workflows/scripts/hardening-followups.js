...
const COMMON = `zclassic23 is one C23 binary (full ZClassic node + wallet), consensus-parity with zclassicd. Offline simnet (lib/sim) mines blocks + builds real txs deterministically (no mainnet needed). HARD RULES: consensus parity inviolable; every malloc via zcl_malloc, every error return LOG_*, every write AR lifecycle; files < 800 lines (app/+config ENFORCED); every thread supervised; NEVER touch a live datadir / the mint producer / stop zclassicd. If your worktree cannot link, run make worktree-init first. OWN isolated worktree — commit to THIS branch only, do NOT push/merge/touch others. Self-gate: make build-only + focused test group + make lint (+ agent_impact_rules.def per changed .c; reconcile DOC-COUNTS if a dumper). Final message = structured data, technical; report the branch name.`
...
LANE stage-latency telemetry (OS observability). Per-reducer-stage p50/p99 latency is not exposed over Prometheus. Add a per-reducer-stage latency surface: register a stage_metrics Prometheus histogram (or gauge set) fed from the EXISTING per-stage step_us EWMA / step counters (grep <stage>_stage_step_us_ewma, lib/metrics/src/stage_metrics.c), exposing fold/sync p50-p99 over time via /metrics, complementing the rom_compile dumper. If a dumper field is added,  Add a focused test asserting the metric is populated from synthetic stage step timings. Files: lib/metrics/src/*.c + at most one wiring point + a test. Measured/real timings only — no fabricated numbers.`,
