...
const COMMON = `zclassic23 is one C23 binary (full ZClassic node + wallet), consensus-parity with zclassicd. Offline simnet (lib/sim) mines blocks + builds real txs deterministically (no mainnet needed). HARD RULES: consensus parity inviolable; every malloc via zcl_malloc, every error return LOG_*, every write AR lifecycle; files < 800 lines (app/+config ENFORCED); every thread supervised; NEVER touch a live datadir / the mint producer / stop zclassicd. If your worktree cannot link, run make worktree-init first. OWN isolated worktree — commit to THIS branch only, do NOT push/merge/touch others. Self-gate: make build-only + focused test group + make lint (+ agent_impact_rules.def per changed .c; reconcile DOC-COUNTS / EXPECTED_DIAGNOSTICS_DUMPERS if a dumper). Final message = structured data, technical; report the branch name.`
