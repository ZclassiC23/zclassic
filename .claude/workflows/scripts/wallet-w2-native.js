...
HARD RULES: consensus parity inviolable (no tx-validity / prover change); every malloc via zcl_malloc, every error return LOG_*, every write AR lifecycle; files < 800 lines (app/+config ENFORCED); NEVER touch a live datadir (~/.zclassic-c23), the mint producer (~/.zclassic-c23-mint), or stop zclassicd. If your worktree cannot link, FIRST: cp -a /home/rhett/github/zclassic23/vendor/lib vendor/. You are in your OWN isolated git worktree — commit to THIS branch only; do NOT push/merge/touch others. Self-gate: make build-only + make t-fast ONLY=<group> + make lint (+ agent_impact_rules.def per changed .c; reconcile DOC-COUNTS). Final message = structured data, technical; report the branch name.`
