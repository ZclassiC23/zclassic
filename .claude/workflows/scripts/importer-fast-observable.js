...
HARD RULES: consensus parity inviolable; all-or-nothing atomicity (any anomaly rolls back leaving cursors POSITIVE + blockers raised — a safe halt, never a partial flip; do NOT weaken this to go faster); every malloc via zcl_malloc, every error return via LOG_*, every write via AR lifecycle; files under 800 lines (app/+config ENFORCED, lib WARN); NEVER touch a live datadir, NEVER stop zclassicd or the mint producer (you MAY read $HOME/.zclassic READ-ONLY). If your worktree cannot link (undefined vendor symbols / missing vendor/lib/*.a), FIRST run: cp -a /home/rhett/github/zclassic23/vendor/lib vendor/ (gitignored, absent in a fresh worktree). You are in your OWN isolated git worktree — work only here; commit green work to THIS worktree's branch; do NOT push, merge to main, or touch any other worktree. Self-gate: make build-only, the focused test group via make t-fast ONLY=<group>, make lint (add agent_impact_rules.def mapping for every changed .c). Final message = structured data for the orchestrator, precise + technical; report the branch name.`
