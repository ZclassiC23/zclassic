...
FUZZ_TARGETS = $(BIN_DIR)/fuzz_block $(BIN_DIR)/fuzz_script $(BIN_DIR)/fuzz_p2p $(BIN_DIR)/fuzz_http $(BIN_DIR)/fuzz_tx_bundle
FUZZ_CI_TIME ?= 60
...
.PHONY: fuzz_block fuzz_script fuzz_p2p fuzz_http fuzz_tx_bundle
...
clean:
	...
	rm -f test_zcl test_parallel zclassic23 zclassic-cli zcl-rpc zcl-nodectl \
	    ...
	    fuzz_block fuzz_script fuzz_p2p fuzz_http fuzz_tx_bundle test_zcl_cov
