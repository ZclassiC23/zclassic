...
- Read model: `app/services/src/vault_read.c` (+ `vault_read.h`, which states
  the two rules the design turns on).
- Command surface: `config/commands/vault.def`, handlers in
  `tools/command/native_vault_command.c` and `native_vault_session_command.c`.
- Proofs: `lib/test/src/test_vault_{read,dispatch,session}.c`.
...
<!-- claim: file-present app/services/src/vault_read.c -->
<!-- claim: file-present config/commands/vault.def -->
<!-- claim: symbol-present source_primitive app/services/include/services/vault_read.h -->
...
- `struct fr` — 256-bit, `uint64_t d[4]`, Montgomery form
  (`lib/sapling/include/sapling/fr.h`); the BLS12-381 scalar field.
- `struct fp` — 384-bit, `uint64_t d[6]` (`lib/sapling/include/sapling/bls12_381.h`);
...
- One file-static `struct bigint` — 288-bit, nine 32-bit limbs, private to
  `lib/sapling/src/jubjub.c` and reachable from nowhere else.
...
workflow; when it arrives it belongs in `lib/crypto/` (the primitives module),
not in `lib/sapling/`, which is the Sapling *protocol* layer that consumes it.
...
<!-- claim: symbol-absent bigint lib/crypto/* # no shared big-integer type yet -->
<!-- claim: symbol-present bigint lib/sapling/src/jubjub.c # the only one today, file-static -->
<!-- claim: file-present lib/sapling/include/sapling/fr.h -->
...
- Contract: `lib/kernel/include/kernel/service_manifest.h`
  (`zcl.service_manifest.v1`) — pointer-free, so a manifest is hashable and
  comparable; validation lives in `lib/kernel/src/service_manifest.c`.
- Catalog: `config/services/catalog.def`, consumed by
  `config/src/service_catalog.c` via `ZCL_SERVICE_ENTRY(...)`.
...
- Proof: `lib/test/src/test_service_manifest.c`.
...
<!-- claim: file-present config/services/catalog.def -->
<!-- claim: symbol-present ZCL_SERVICE_ENFORCEMENT_SHADOW config/services/catalog.def -->
<!-- claim: symbol-absent ZCL_SERVICE_ENFORCEMENT_ACTIVE config/services/catalog.def # still shadow-only -->
