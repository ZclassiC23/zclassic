...
| S6 | Read-only Noise-bound DHT, persisted contacts, diagnostic dumper | S2 | **complete and gate-proven 2026-08-03; not deployed** — chain-bound ZID/online-key/Noise delegations; heap-owned k=16/alpha=3 table with deterministic incumbent probing, 1,024-contact cap, strict canonical `contacts.v2`, and cold revalidation; authenticated bounded FIND_NODE/NODES rides the existing `zpkgswm` + negotiated Noise session (no new socket stack); 64 peer slots, three active queries, 30 s ceiling, replay/rate/outbound caps and named rejection counters; read-only `zcode.network.status|peers|find` plus `zcode_dht` diagnostics (including node ID, contacts, buckets used, probes/queries, frame counters, persistence, last error; never keys/addresses/operator-diversity claims). `make test-zcode-dht-acceptance` proves two independent clean regtest nodes, chain anchoring/finality, capability upgrade, mutual XOR-ordered discovery, exact hostile-frame rejection, canonical persistence/cold reload, short-disconnect retention, and reauthentication. Focused DHT/Noise/transport/argv, yardsale/store + store stress, lint, cold full suite, science acceptance, and both reproducibility gates are green. Provider/root records and STORE/ack/replication are S7, not smuggled into S6. |
