...
| S6 | Read-only Noise-bound DHT, persisted contacts, diagnostic dumper | S2 | **complete and gate-proven 2026-08-04; not deployed** — genuine deterministic iterative Kademlia: k=16 XOR shortlist, alpha=3 global query budget, eight fairly queued lookups, explicit unverified/unreachable/authenticated/queried/in-flight/responded/failed states, stable/target/timeout termination, and a 30 s per-lookup ceiling. Address-free NODES IDs request only a bounded existing-network dial through an already accepted signed, chain-bound ZENDP endpoint; only a matching Noise session + delegation promotes or returns a node. Replay storage covers the full rate-bounded window (138/session), all operational deadlines are monotonic, key rotation is master-authorized and monotonic, rejected parsers leave zero output, stale candidates are revalidated, and delayed beacons use logarithmic ancestry. `make test-zcode-dht-acceptance` proves seven independent identities on a sparse chain plus one alternate edge: no direct table/SQL insertion or all-to-all topology, at least three rounds, XOR progress, bounded traffic, broken-nearest-path recovery, eight concurrent callers, replay saturation, canonical multi-contact persistence, cold restart, and reauthentication. Diagnostics expose rounds, shortlist composition, queue wait, termination counts, and ancestry span without keys or addresses. Focused DHT/Noise/transport/argv/connman/RPC, yardsale/store + both store stress groups, 132 lint gates, cold uncached 887/887 eligible groups, LTO, science acceptance, and both reproducibility gates are green. Provider/root records and STORE/ack/replication remain S7 and were not added. |
