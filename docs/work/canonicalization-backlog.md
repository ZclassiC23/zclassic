...
| `not_supported_by_znam_v1`, `append_only_v2` | `app/controllers/src/name_controller.c`, `agent_interface_controller.c` | API response string literals naming the **ZNAM protocol version** / **agent-contract version** they describe — external-facing protocol labels, not internal code names. |
...
| `run_gate_script_with_env2`, `ace_hash2`, `build_file_cb2` | `lib/test/src/test_make_lint_gates.c`, `test_active_chain_extend.c`, `lib/codeindex/src/codeindex_build.c` | File-scoped `static` test/build helpers distinguishing "the second callback/env-count variant used later in the same file" — zero public-API exposure, zero cross-file callers. `run_gate_script_with_env2` even has a doc comment: "Like run_gate_script_with_env but exports TWO env vars." Cosmetic at most; not worth a backlog slot given zero blast radius. |
