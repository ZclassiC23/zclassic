...
Read docs/work/next-wave-plan.md section 3. Several models copy fixed-size fields (16/32/43-byte hashes, scripts) out of SQLite rows that are populated from PEER GOSSIP, using memcpy without checking sqlite3_column_bytes — a malformed/short blob row causes an out-of-bounds read.
...
Read docs/work/next-wave-plan.md section 4. Raw stdout writes from library/service code corrupt the -mcp stdio JSON protocol and hide diagnostics from node.log.
