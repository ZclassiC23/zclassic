...
- Bring the remaining broad plans under the resident proof bound without
  ...
  occurrences. The latest clean replay was partial because three paths (18
  weighted occurrences) hit the 10-second verdict bound; two other plans (11
  weighted occurrences) still exceed 32 immediate exact groups and are
  refused before proof compilation/linking. Its bound samples measured p50
  7.942 s and p95 10.091 s, with 0/61 trustworthy under five seconds in that
  cache state. Source-byte instrumentation remains open.
