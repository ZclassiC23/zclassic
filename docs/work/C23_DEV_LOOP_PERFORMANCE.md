...
- Bring the remaining broad plans under the resident proof bound without
  ...
  occurrences. The latest stable replay is complete at p50 4.808 s / p95
  5.295 s and 26/61 (42.62%) trustworthy under five seconds; the two broad plans
  (11 occurrences) still refuse before proof compilation/linking. Admitted
  paths now cluster near the threshold, so the next slice must reduce the two
  complete-graph links or another measured fresh proof body rather than move
  evidence out of the immediate tier. Source-byte instrumentation remains
  open.
