...
| L7 | ~~**Shielded anchor-membership is a stub**~~ — **CLOSED, do not re-dispatch** | `lib/coins/src/coins_view.c` (`coins_view_cache_check_shielded_requirements`), enforced at `app/jobs/src/utxo_apply_anchors.c:227` | **was: real fork vector, parity-RESTORING — now implemented** | **This row is historical.** The membership rule is no longer absent: the function resolves every JoinSplit and Sapling Spend anchor through `coins_view_get_anchor()`, treats the empty-tree root as a per-pool constant, and reproduces zclassicd's per-transaction `intermediates` map (an anchor made by an earlier JoinSplit of the *same* tx is valid; one made by an earlier tx of the same block is not yet pushed) — the `HaveShieldedRequirements` shape from `coins.cpp:565-604`. Enforcement is on the **reducer fold path** (`utxo_apply_anchors.c`), not the legacy `connect_block()` path the original row cited; `coins_view_cache_have_joinsplit_requirements()` is now a thin bool wrapper. Pinned by `lib/test/src/test_parity_lockin_anchor_membership.c`, registered as `X(parity_lockin_anchor_membership)`. Being parity-RESTORING, closing it never needed a replay permission. |
