...
The RAM-network simulation harness is built on `lib/sim/src/seed_tape.c`
...
- `lib/sim/src/simnet.c` — single-node RAM harness over real
  `connect_block(..., expensive_checks=false)`.
- `lib/sim/src/simnet_cluster.c` — multi-node cluster with deterministic
...
- `lib/sim/src/simnet_byzantine.c` — adversarial peers and typed-blocker
  ...
  `simnet_byzantine`.
- `lib/sim/src/simnet_wire.c` — in-memory wire-frame transport (see
...
- `lib/sim/src/simnet_mempool.c` + `simnet_contract.c` — RAM mempool and
