...
**F1. `.failed`-marker crash-loop when the marker can't be written** — **FIXED 2026-06-30.** The policy now lives in `config/src/boot_snapshot_failure_memory.c`: autodetect and explicit snapshot seeds both write durable failure memory before entering the fail-closed loader; existing `.failed` markers skip the seed; and marker-unwritable/path-too-long cases degrade to normal P2P sync instead of gambling on a seed whose failure could not be remembered. Regression: `test_boot_snapshot_failure_memory`.

**F2. Record-count-mismatch / truncated-snapshot has no failure memory at all** — **UPDATED 2026-06-30.** The first `uss_open` SHA3-verify still fails closed inside `boot_refold_staged.c`, but both autodetect and explicit `-load-snapshot-at-own-height` now create the `<snapshot>.failed` marker before calling the loader. A truncated or mismatched snapshot therefore crashes at most once under systemd, then the next boot skips that seed and falls back to P2P unless the operator deletes the marker to retry it.
