...
**Status:** historical design (no production code in this branch). **Scope:**
the LB-1 hot-path wiring tranche. **Goal:** the then-shipped-but-unwired engine
(`lib/validation/{thread_pool,verify_queue}.{c,h}`)
