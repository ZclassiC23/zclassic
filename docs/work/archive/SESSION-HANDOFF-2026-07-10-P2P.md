...
| Groth16 C23 prover cure | `fix/groth16-c23-prover` @ `13b557742` | output circuit (7827/7821) + density-filtered MSM FIXED + unit-pinned. Round-trip WIP bisection harness committed (`ddc832a4e`, self_verify still FALSE); spend circuit not started. Ground-truth traces preserved on-branch at `reference/groth16-traces/`. | **FULL DOC: `docs/work/GROTH16-C23-PROVER-CURE.md`** — root cause, exact fixes, and the resume checklist (run the `groth16_selfverify` bisection, first diverging stage = the bug; then port the spend circuit). Ship-target that retires the librustzcash bridge. |
...
| Storefront service | `feat/storefront-service` @ `500dd86fe` | impl COMMITTED + **independently VERIFIED merge_ready** (23 files: listing resource via migration v26, order lifecycle, 5 MCP tools, GET-only REST; reuses the proven store E2E path; 0/508 green, lint clean). Fixed 2 real pre-existing bugs: NODE_DB_MAX_SCHEMA/NODE_DB_SCHEMA_LATEST must bump in lockstep, and db_store_product_save never returned last_insert_rowid. | **READY TO MERGE** — additive conflict likely in test_mcp_controllers.c tool counts + docs/CODEBASE_MAP.md (resolve as union). Distinct from the example-12 demo lane below. |
