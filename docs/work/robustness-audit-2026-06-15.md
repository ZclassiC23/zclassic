...
6-surface adversarial audit of the critical path — reducer engine, repair ladder,
coin/UTXO apply+backfill, boot/import/recovery, defensive-coding, concurrency/lifecycle.
Every finding verified against the cited code.

## Architecture verdict — SOUND, no refactor

Log-as-truth / single-reducer holds; the recurring failure is **torn UTXO sets
INSTALLED at import with no cryptographic proof**, detected ~80k blocks later when
forward apply spends a missing coin. Root cause + cure (write-time SHA3 import gate;
then delete the ~6,040-LOC ladder leaf-first) are canonical in
`rock-solid-program` §1, `import-gate-spec`, and `tenacity-roadmap` — not restated here.
...
Gate at each step: lint 37/37, full LTO build clean, `test-parallel` 0/424.

## Correction — REP-1 was WRONG

`stage_reconcile_clamp_tip_finalize_to_floor` (`app/jobs/src/stage_repair.c`) is
**NOT dead** — live production caller at `config/src/boot.c:3260-3274`. **Do not
delete this TU.** Ladder-deletion sequencing must re-derive the caller graph with
`config/` in scope.

## Deferred — safe, want their own tested pass (NOT started)
...
- **Clear the live wedge:** node still on the OLD binary, wedged at h=3,145,594 on
  the coin tear. Remedy: deploy the merged binary + wipe + two-step cold re-import
  (copy-prove on the fixture first). Unblocks MVP C3/C6/C8. BOO-1's gate protects
  FUTURE imports only.
...
- **COI-2 (P1, forward fork-risk):** a within-block **cross-transaction
  ...
  History-safe (no such block exists on the immutable chain), but a crafted forward
  block would fork. Fix RESTORES parity: scan recorded spent[] for a matching
  outpoint before recording; `delta_fail` with a **non-repairable** status — NOT
  `value_overflow` (the ladder treats that as repairable). Per the Consensus Parity
  Doctrine, **must be validated by a full-history replay** against the real chain
  and pinned by a `test_consensus_parity` golden before shipping. Do not land
  without the replay.
