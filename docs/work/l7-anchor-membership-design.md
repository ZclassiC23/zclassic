...
> chain history. `lib/coins/src/coins_view.c:477-486` is a literal `return true`
...
| 2 | Inline fold writer `app/jobs/src/utxo_apply_anchors.c` (new file — `utxo_apply_stage.c` is 798 lines, at the E1 ceiling): maintain both incremental trees inline, append this block's `cm`s, compute root, `anchor_kv_add` in the **same stage txn** — mirrors `PushAnchor`. Subsumes the standalone `sapling_tree_rebuild`. | consensus-adjacent |
