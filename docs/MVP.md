...
|---|---|---|---|
| 1 | **Single-binary install on clean Ubuntu/Debian** | Target: clean container, `make install && systemctl --user start zclassic23`, exit 0 — **hermetic proxy: `make ci-install`** (not in the `make ci` target): drives the real `make install` (the two binaries + the systemd `--user` unit) to a throwaway `/tmp` prefix, starts ONE node FROM that prefix as a fully isolated regtest node (unique `/tmp` datadir + 39xxx non-live ports + `-connect=39999` dead sink, via the audited `tools/scripts/isolated_node_env.sh`), polls RPC readiness, asserts the installed binary answers + bound only non-live ports, and cleans up. Also in the one-command local aggregate `make mvp-verify` (locally-verified, not hermetic-✅). The clean-OS half of the claim now has its own real proof: **`make ci-install-container`** (`tools/scripts/ci_install_container_gate.sh`) runs the self-contained binary inside a clean `ubuntu:24.04` (glibc 2.39 / GLIBCXX 3.4.33 — at/above the binary's real **triple** symbol-version floor) with ONLY `build/bin` mounted read-only — no compiler, no repo source — and asserts the installed binary answers `getblockcount` as an isolated regtest node, proving it carries no hidden host dependency. It is docker-gated and SKIPs cleanly (exit 2→SKIP) when docker is unreachable, exactly like `mvp-onion-local`; its inner spawn/poll logic is host-validated. The portability floor itself is now a **hermetic gate IN `make ci`**: **`make ci-symbol-floor`** (`tools/scripts/ci_symbol_floor_gate.sh`) statically asserts via `objdump -T`/`ldd` that every dep resolves and the binary's max required symbol in each family stays ≤ the documented floor **GLIBC_2.38 / GLIBCXX_3.4.30 / CXXABI_1.3.9** (the real floor is a triple — libstdc++/libgcc_s, not glibc alone — so an Ubuntu 20.04 with GLIBCXX_3.4.28 would NOT satisfy it; a regression that silently pulls a newer symbol fails the build loud). Still ◐: the container run-proof is pending docker access on the build host (a one-time owner action — docker-group membership), and a real `systemctl --user start` on a clean container is still not exercised. | ◐ |
