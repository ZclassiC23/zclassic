...
| 3 | **Cold-start sync to tip in <10 min** | Fresh datadir → `zcl_syncstate.phase=ready` within 10 min on 100 Mbps link — test: `lib/test/src/test_cold_start_sync.c` (**CI slice: `make ci-mvp-gates`** — drives the sync FSM to `at_tip` in ~7s; does **not** download/validate a real 3M-block chain). The seed-authority sub-proof is `make mvp-coldstart-local`: fixture-gated, secure operator bundle (`block_index.bin` + `utxo-seed-*.snapshot`), fresh `/tmp` datadir, `-load-snapshot-at-own-height`, and >1M UTXOs self-verified into the durable authority surface in <90s. The full C3 proof command is now `make mvp-coldstart-to-tip-local`: same bundle, fresh datadir, dial a serving zclassic23 peer, and reach that peer's captured tip within the 10-minute MVP budget. Legacy `consensus_snapshot.db` import remains only a checkpoint-height fallback; above-checkpoint peer snapshots are intentionally refused because there is no in-binary root. **Empirical finding (2026-06-17, old snapshot-only `tools/scripts/cold_start_to_tip_probe.sh`):** a fresh snapshot-only node dialing a live serving peer did NOT reach `at_tip` because the snapshot import path set `coins_best_block` without seeding `coins_kv` / `coins_applied_height`. **Update (2026-07-08):** `make mvp-coldstart-local` run-passed the bundle seed proof on this host in 17s (`count=1344903`, `body SHA3 OK`, H*=3155842), and the full tip probe harness now uses the same bundle path. C3 remains ◐ until `make mvp-coldstart-to-tip-local` run-passes. | ◐ |
