...
- **#1** (single-binary install) now has the hermetic `make ci-install` proxy
  gate (in the `mvp-spawn` push/PR CI job, not the `make ci` target) — install-to-throwaway-`/tmp`-prefix +
...
- **#6** (7-day soak) has a bounded proxy (`make soak-ci`) gated on push/PR via
  the `mvp-spawn` job, but no test in the default `make ci` target and no real
