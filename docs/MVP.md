...
| 3 | **Cold-start sync to tip in <10 min** | Fresh datadir → `zclassic23 core sync status` reports `phase=ready` within 10 min on 100 Mbps link — test: `lib/test/src/test_cold_start_sync.c` (**CI slice: `make ci-mvp-gates`** — drives the sync FSM to `at_tip` in ~7s; does **not** download/validate a real 3M-block chain). The assisted-seed sub-proof is `make mvp-coldstart-local`: fixture-gated operator bundle (`block_index.bin` + `utxo-seed-*.snapshot`), fresh `/tmp` datadir, `-load-snapshot-at-own-height`, and >1M UTXOs body-digest verified and installed in <90s. That proves byte integrity and operational seeding, not sovereign state provenance. The full C3 proof command is now `make mvp-coldstart-to-tip-local`: same bundle, fresh datadir, dial a serving zclassic23 peer, and reach that peer's captured tip within the 10-minute MVP budget. Legacy `consensus_snapshot.db` import remains only a checkpoint-height fallback; above-checkpoint peer snapshots are intentionally refused because there is no in-binary root. **Empirical finding (2026-06-17, old snapshot-only `tools/scripts/cold_start_to_tip_probe.sh`):** a fresh snapshot-only node dialing a live serving peer did NOT reach `at_tip` because the snapshot import path set `coins_best_block` without seeding `coins_kv` / `coins_applied_height`. **Update (2026-07-08):** `make mvp-coldstart-local` run-passed the bundle seed-integrity proof on this host in 17s (`count=1344903`, `body SHA3 OK`), and the full tip probe harness now uses the same bundle path. C3 remains ◐ until the applicable complete shielded-history gates and `make mvp-coldstart-to-tip-local` both pass. | ◐ |
...
| 6 | **7-day soak with zero operator intervention** | Live node + synthetic load for 168h: no manual restarts, RSS plateau. **Hermetic proxy: `make soak-ci`** (not in `make ci`) exercises scoring mechanics only. Real evidence comes from `make soak-7day` and `make soak-evidence-report` (`MET\|NOT_MET\|INSUFFICIENT`). **Current status:** canonical is wedged below tip on incomplete shielded history (verify the live H\* via `zclassic23 status` / `zclassic23 dumpstate reducer_frontier`; `docs/HANDOFF.md` holds current state) and the formal judge is `NOT_MET` after operator intervention. No current canonical wall time is clean soak evidence. A new window starts only on a sovereign, exact-same-height-parity candidate with gap ≤1, complete security posture, continuous evidence, and zero manual restarts; one-block lookahead or process uptime alone never earns C6. | ◐ |
...
(the 168h soak window — canonical is currently wedged, so no clean window is
accruing), **C8** (an exact
...
Soak (#6) and parity (#8) both also need live forward progress. Canonical is
currently wedged on incomplete shielded history, so its wall time is not a clean
window. After the complete state cure is copy-proven and deployed, restart C6
from an exact-parity, complete-security-posture candidate. #6's `make ci-stress` soak proxy
