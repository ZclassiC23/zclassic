...
| Commerce | `store_transparent_payment`, `store_shielded_payment`, `yardsale_atomic_purchase`, `market_purchase` | The exact jointly signed Yardsale controller broadcast is isolated-mined from three real fixture UTXOs with exact settlement and fee accounting. Shielded store pay is isolated-only. File-market plan/commit/retrieve completes authenticated payment, verified assembly, and atomic publication. |
...
The current complete inventory is **34/34 isolated cases passing**, with **30
