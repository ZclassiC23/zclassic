...
| **`check-zcode-package-registry`** | HARD | Re-derives the content manifest, unsigned release/signing root, recipe, target-inclusive exact dependency lock, and API capsule roots for `zclassic23/base`, `zclassic23/sha3`, and `zclassic23/codec` directly from their authoritative `lib/<module>` trees. The fixed publisher, sequences 1/2/3, empty reward address, `zclassic-main`, exact roots, and direct dependency DAG are projected in `config/zcode_package_registry.def`. Drift, an added dependency, or a package source/API change without regenerating the projection fails. Production source ownership remains the unique `LIB_MODULE` row checked independently by `check-lib-module-order`, so the monolith compiles each package source exactly once. No baseline or override. |
