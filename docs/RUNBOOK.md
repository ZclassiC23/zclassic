...
3. **Only `connecting` peers:** prefer fresh addnodes from known ZClassic peers. The compact peer incident view shows `primary_host_issue` and `top_host_incidents` first, then reconnect pressure, duplicate host groups, current open/handshaked duplicate groups, direction, handshake age, advertised height, service summary, bootstrap readiness, and whether a peer is currently useful for bootstrap. The full `peer_lifecycle.sources[]` view shows whether failures concentrate in `addnode`, `addrman`, `manual`, `zcl23_db`, or `inbound`; the coordinator dump distinguishes TCP failures (`addnode_tcp_failures`) from post-connect protocol/handshake failures (`addnode_protocol_failures`).
