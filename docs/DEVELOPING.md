...
2. **Hot-swap tiers for the fast inner loop — three of them, pick by surface:**
   ...
   - **Bounded non-consensus C — candidate first, proof next:** when exactly one to 32 ordinary C translation units change, the resident owner snapshots the source CAS, compiles only those units from its frozen `DEV_RESTART` plan, reuses any still-current prior overlays, directly relinks an isolated executable, rechecks the CAS, and runs its bounded command-runtime probe. This starts no Make or shell process and reports `candidate_ready`, `runtime_published=false`, and `proof_complete=false`. It deliberately does not restart a node or call the candidate proven; mapped resident tests are the next authority layer. Headers, build-graph edits, consensus-risk paths, and oversized batches stay on the conservative proof path. Eligible stateless handlers may instead use the shared-object probe path without publication.
