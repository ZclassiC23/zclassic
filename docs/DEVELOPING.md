...
1. **Persistent watcher (default-safe, hot when requested):** run `zclassic23-dev dev begin` once. Humans may then **Edit `.c`** and use the exact `dev drive` command returned by `begin`; it carries the current cursor plus `wait_for_edit:true` and returns the first action-changing diagnostic or owner-bound story. An AI should instead attach once with `zclassic23-dev dev loop events --after=<cursor> --format=jsonl`, continue thinking immediately after every edit, and consume resumable `IMPACT_READY`, `COMPILE_*`, `STORY_*`, and heartbeat records from that one process. `STORY_RED` includes a minimal diagnostic capsule. The bounded `dev loop wait` and `dev drive` surfaces remain useful for inspection and human convenience; neither is required per edit. All waits use inotify, never polling. `begin` defaults to verify mode and idempotently reattaches to the checkout's one warm service. The explicit controls remain `zclassic23-dev dev loop ensure` (or `make dev-watch`) for verify mode and `zclassic23-dev dev loop ensure --input='{"mode":"auto"}'` for the owner-gated future publication path. Auto remains structurally contained until the immutable source epoch, complete proof receipts, resident CAS, and rollback are one durable transaction; requesting it never silently falls back or implies that a runtime was changed. `dev status` remains the compact latest-state surface.
   The ring publishes reflex events before the SHA3-sealed append-only journal acknowledges storage. Exact affected proof and conservative integration continue afterward; a newer save publishes its next impact while cancelling obsolete proof. Every green story seals the complete one-way async boundary—candidate/source epoch, affected component, action/proof inputs, and the exact focused-evidence root—before it emits `PROOF_PENDING`. Later proof may add receipts but cannot delay, rewrite, or invalidate that reflex result. No agent chooses a test or launches `dev ff`. A passed conservative receipt says `proof_complete:true`, `proof_scope:source_wide_compile_tests_lint_fast`, and `proof_reuse_scope:exact_source_id_sha256`. Any deferred, superseded, rejected, probe-only, or publication receipt says `proof_complete:false` and is never represented as passed proof. Reproduce the 20-green-plus-one-red latency/firewall gate with `make reflex-reactor-bench`; measure recent-edit coverage with `make reflex-coverage-audit`. Architecture and measurements are in [`work/REFLEX_REACTOR.md`](work/REFLEX_REACTOR.md).
...
2. **Hot-swap tiers for the fast inner loop — three of them, pick by surface:**
   ...
   - **Static authority shell + pure `HOT_SHADOW` core:** `config/hotswap_shadow_owners.def` maps high-churn controllers, command handlers, build machinery, and evidence/package shells to a pure service island. The changed shell receives an exact `-fsyntax-only` semantic compile; it is never linked, loaded, or granted runtime authority. Only its mapped caller-owned calculation/view core runs the resident-frozen KAT in a disposable fork. Pure helper TUs may join that candidate only through an explicitly linted `HOTSHADOW_SERVICE_MEMBERS` row. Git, Make, SQLite, full links/suites, ZVCS publication, and network/DHT work remain outside this path.
...
**Enumerating the 157 dumpstate subsystems is `zclassic23 statecatalog`**, not
