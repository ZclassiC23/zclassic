...
1. **Persistent watcher (default-safe, hot when requested):** the short agent loop is `zclassic23-dev dev begin` once, then **Edit `.c`**, then `zclassic23-dev dev drive`. `begin` defaults to verify mode and idempotently reattaches to the checkout's one warm service; `drive` waits for the next exact verdict and returns one compact proof, ZVCS, publication-blocker, and next-action view. The explicit controls remain `zclassic23-dev dev loop ensure` (or `make dev-watch`) for verify mode and `zclassic23-dev dev loop ensure --input='{"mode":"auto"}'` for the owner-gated future publication path. Auto remains structurally contained until the immutable source epoch, complete proof receipts, resident CAS, and rollback are one durable transaction; requesting it never silently falls back or implies that a runtime was changed. `dev status` and `dev loop wait` remain the detailed receipt surfaces.
   A restart-class edit first emits `feedback_ready` after its exact affected tests, then the same watcher automatically continues the conservative integration proof. The first compact `drive` reply says `publication_stage:PROOF_PENDING`; no manual test selection or `dev ff` launch is required. A passed conservative receipt says `proof_complete:true`, `proof_scope:source_wide_compile_tests_lint_fast`, and `proof_reuse_scope:exact_source_id_sha256`. That is the watcher-owned compile, source-wide test, and lint-fast proof for the displayed source ID; the exact next action is to keep editing, not to launch a duplicate `make t-fast`. Any deferred, superseded, rejected, probe-only, or publication receipt says `proof_complete:false` and is never represented as passed proof.
