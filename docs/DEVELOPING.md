...
1. **Persistent watcher (default-safe, hot when requested):** the short agent loop is `zclassic23-dev dev begin` once, then **Edit `.c`**, then `zclassic23-dev dev drive`. `begin` defaults to auto mode and idempotently reattaches to the checkout's one warm service; `drive` waits for the next exact verdict and returns one compact live/fallback, proof, ZVCS, publication-blocker, and next-action view. The explicit controls remain `zclassic23-dev dev loop ensure` (or `make dev-watch`) for verify mode and `zclassic23-dev dev loop ensure --input='{"mode":"auto"}'` for auto mode. One allowlisted stateless island takes the resident compile→link→probe→atomic publish path; every other edit is downgraded to the contained proof lane. `dev status` and `dev loop wait` remain the detailed receipt surfaces.
   A passed conservative receipt says `proof_complete:true`, `proof_scope:source_wide_compile_tests_lint_fast`, and `proof_reuse_scope:exact_source_id_sha256`. That is the watcher-owned compile, source-wide test, and lint-fast proof for the displayed source ID; the exact next action is to keep editing, not to launch a duplicate `make t-fast`. Any deferred, superseded, rejected, probe-only, or publication receipt says `proof_complete:false` and is never represented as passed proof.
