...
Every path, schema id, and example is transcribed directly from the `.def`
source; verify live against a running binary with:
...
The registry is rooted at 6 top-level choices (`status`, `core`, `app`,
`dev`, `ops`, `discover`); run `zclassic23 discover help` with no path for the
current leaf/branch counts rather than trusting a pinned number here.
...
| `core.chain.wait.height` | `core chain wait height --height=N` | **planned** | read / operator | `height,timeout_ms` (**`height`**) | `zcl.wait_result.v1` | Wait until the tip reaches a target height — *needs the job protocol, not yet implemented* |
| `core.chain.wait.blocker` | `core chain wait blocker` | **planned** | read / operator | `blocker,timeout_ms` (**`blocker`**) | `zcl.wait_result.v1` | Wait until a named blocker is raised/cleared — *not yet implemented* |
| `core.chain.wait.halt` | `core chain wait halt` | **planned** | read / operator | `timeout_ms` | `zcl.wait_result.v1` | Wait until the node halts on a named blocker — *not yet implemented* |
...
| `core.consensus.block.invalidate` | `core consensus block invalidate --hash=<h>` | **planned** | destructive / core-recovery / **owner**, plan-commit | **`hash`** | `zcl.block_mutation.v1` | Mark a block invalid and reorg away from it — *confirmation handshake not yet implemented* |
| `core.consensus.block.reconsider` | `core consensus block reconsider --hash=<h>` | **planned** | mutate / core-recovery / **owner**, plan-commit, reversible | **`hash`** | `zcl.block_mutation.v1` | Clear an invalid mark and reconsider a block — *not yet implemented* |
...
| `core.network.peers.add` | `core network peers add --address=<ip:port>` | **planned** | mutate / core-recovery / operator | **`address`** | `zcl.peer_add.v1` | Add an outbound peer connection — *not yet implemented* |
...
| `core.wallet.address.new` | `core wallet address new` | **planned** | mutate / wallet / **owner** | none | `zcl.wallet_address.v1` | Derive and persist a new transparent address — *not yet implemented* |
...
| `core.wallet.address.import` | `core wallet address import --address=<addr>` | **planned** | mutate / wallet / **owner** | **`address`** | `zcl.wallet_address.v1` | Import a watch-only address — *not yet implemented* |
| `core.wallet.address.export-key` | `core wallet address export-key --address=<addr>` | **planned** | read / **owner**, plan-commit | **`address`** | `zcl.wallet_privkey.v1` | Export the private key for an address — *owner-gated; not yet implemented* |
...
| `core.wallet.transaction.send` | `core wallet transaction send --input='<obj>'` | **planned** | mutate / wallet / **owner**, plan-commit | `address,amount,idempotency_key` | `zcl.wallet_send.v1` | Build, sign, and broadcast a payment — *spend handshake not yet implemented* |
| `core.wallet.shielded.address` | `core wallet shielded address` | **planned** | mutate / wallet / **owner** | none | `zcl.shielded_address.v1` | Derive a new shielded address — *not yet implemented* |
| `core.wallet.shielded.balance` | `core wallet shielded balance --address=<zaddr>` | **planned** | read / operator | **`address`** | `zcl.shielded_balance.v1` | Shielded balance for one address — *arg mapping not yet implemented* |
| `core.wallet.shielded.notes` | `core wallet shielded notes` | **planned** | read / operator | none | `zcl.shielded_notes.v1` | List spendable shielded notes — *not yet implemented* |
| `core.wallet.shielded.send` | `core wallet shielded send --input='<obj>'` | **planned** | mutate / wallet / **owner**, job, plan-commit | `from,to,amount,idempotency_key` | `zcl.shielded_send.v1` | Send a shielded payment (`z_sendmany`) — *not yet implemented* |
...
| `core.wallet.backup.now` | `core wallet backup now` | **planned** | mutate / wallet / **owner**, idempotent | none | `zcl.wallet_backup.v1` | Take a wallet backup now — *not yet implemented* |
...
| `core.wallet.rescan` | `core wallet rescan` | **planned** | mutate / wallet / **owner**, job | `start_height` | `zcl.wallet_rescan.v1` | Rescan the chain for wallet transactions — *not yet implemented* |
| `core.wallet.replay` | `core wallet replay` | **planned** | mutate / wallet / **owner**, job | none | `zcl.wallet_replay.v1` | Replay wallet state from chain — *not yet implemented* |
...
| `core.storage.integrity` | `core storage integrity` | **planned** | read / operator | none | `zcl.storage_integrity.v1` | Verify raw storage integrity — *distinct handler not yet implemented* |
...
| `ops.jobs.list` | `ops jobs list` | **planned** | read / operator | none | `zcl.jobs.v1` | List asynchronous jobs — *native job registry not yet implemented* |
...
| `ops.postmortem.replay` | `ops postmortem replay <id>` | **planned** | read / operator | **`id`** | `zcl.postmortem_replay.v1` | Replay one captured postmortem — *arg mapping not yet implemented* |
| `ops.config.show` | `ops config show` | **planned** | read / operator | none | `zcl.ops_config.v1` | Show effective runtime configuration — *not yet implemented* |
| `ops.config.reload` | `ops config reload` | **planned** | mutate / core-recovery / operator, idempotent | none | `zcl.ops_config_reload.v1` | Reload runtime configuration — *not yet implemented* |
| `ops.recovery.status` | `ops recovery status` | ready | read / operator | none | `zcl.recovery_status.v1` | Refold and recovery progress |
| `ops.recovery.rebuild` | `ops recovery rebuild --depth=100` | **planned** | destructive / core-recovery / **owner**, job, plan-commit | `depth` | `zcl.recovery_rebuild.v1` | Rebuild recent chain state — *not yet implemented* |
