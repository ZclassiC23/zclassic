 ...
 * chain: it is a pure codec (lib/zid is rank 10, below net and vcs).
 * Publishing and fetching live in lib/vcs (vcs/zdesc_swarm.h). Whether
...
/* Exactly the rule enforced in lib/net (onion_hostname_valid,
 * lib/net/src/onion_service.c): 62 chars, ".onion" suffix, and the v3
 * base32 alphabet a-z2-7 for the leading 56. Reimplemented here rather
 * than shared because lib/zid is ranked BELOW lib/net and may not
 * include it (config/lib_module_order.def); the two copies are twins by
 ...
 * since the epoch. Wall clock, not block height — lib/zid is rank 10
 ...
 *            fallback is implemented in the fetch path (lib/vcs), not
 *            left to callers to remember.
 *
 * Frozen by golden vector in lib/test/src/test_zdesc.c: the vector
