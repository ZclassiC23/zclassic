 ...
 *
 * lib/zid already carries the off-chain half of the sovereign identity layer
 ...
 *
 * Strictness (the lib/zanc model, NOT the lib/znam model): every length is
 ...
 * LOCAL copy of MAX_OP_RETURN_RELAY (lib/script/include/script/standard.h):
 * lib/zid sits BELOW lib/script in config/lib_module_order.def, so lib/zid
 * may not reference lib/script. The two are pinned equal by a static
 * assertion in lib/test/src/test_zid.c, which is above both and can include
