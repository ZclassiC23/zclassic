 ...
 *
 * Strictness follows lib/zanc (reject trailing bytes, validate hard), not
 * lib/znam. No logging: these are pure predicates over adversarial chain
 * bytes, so a rejected script is a normal negative result, not an error to
 * record — same contract as lib/overlay and lib/zanc. */
