 ...
 * lib/crypto/include/crypto/common.h (ReadLE16/32/64, WriteLE16/32/64) and
 ...
 *
 *   - lib/zid defines the family THREE times inside ONE module, under
 ...
 *   - config/src/consensus_state_snapshot_candidate.c writes a persisted
 *     8-byte field with `le64_encode`, and
 *     config/src/consensus_state_snapshot_candidate_validate.c reads that
 ...
 *
 * Why lib/base and not lib/crypto
 * -------------------------------
 * Rank, per config/lib_module_order.def. lib/base is rank 1 — the sink
 * every other module is allowed to reach down into. lib/crypto is rank 9,
 ...
 * fixed-width byte-order helper outside lib/base fails `make lint`.
 */
