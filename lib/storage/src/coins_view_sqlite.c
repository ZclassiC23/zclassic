 ...
 * and write directly through the existing prepared INSERT OR REPLACE
 * statement in one BEGIN IMMEDIATE / COMMIT transaction.
 *
 * Used by local_chain_ingest when an embedded UTXO sidecar
 * (utxo_snapshot.dat) is present and SHA3-verified — caller is
 * responsible for ensuring the cache is invalidated after this
 * returns (typically via process_block_set_best_block + a new chain
 * tip transition that drains the cache).
 */
int64_t coins_view_sqlite_bulk_insert(struct coins_view_sqlite *cvs,
                                      const struct utxo_bulk_rec *recs,
                                      size_t n)
{
    if (!cvs || !cvs->db || !cvs->stmt_insert || !recs)
        LOG_ERR("coins_view",
                "bulk_insert: invalid args (cvs=%p db=%p stmt=%p)",
                (void*)cvs, cvs ? (void*)cvs->db : NULL,
                cvs ? (void*)cvs->stmt_insert : NULL);

    const char *txn_begin = cvs->owns_db ? "BEGIN IMMEDIATE"
                                          : "SAVEPOINT coins_bulk";
    const char *txn_commit = cvs->owns_db ? "COMMIT"
                                           : "RELEASE coins_bulk";
    const char *txn_rollback = cvs->owns_db ? "ROLLBACK"
                                : "ROLLBACK TO SAVEPOINT coins_bulk";

    pthread_mutex_lock(&cvs->mutex);

    /* Bump busy timeout — same rationale as batch_write_ex. */
    sqlite3_busy_timeout(cvs->db, 30000);
    ...
    for (size_t i = 0; i < n; i++) {
        const struct utxo_bulk_rec *r = &recs[i];
        sqlite3_stmt *ins = cvs->stmt_insert;
        sqlite3_reset(ins);
    ...
    if (fatal) {
        ...
        sqlite3_busy_timeout(cvs->db, 10000);
        pthread_mutex_unlock(&cvs->mutex);
        return written; /* < n indicates failure */
    }
