 ...
 * count yet?" It lives in lib/policy for that reason and is retunable without
 * a fork.
 *
 ...
 * (lib/validation/include/validation/checkpoint.h:38,43;
 * ZCL_FINALITY_DEPTH at lib/validation/include/validation/main_constants.h:33).
 ...
 * pubkey the anchor names (lib/zid/include/zid/zid.h zid_blinded_key,
 * lib/zid/include/zid/zid_anchor.h ZID_ANCHOR_PUBKEY_LEN). */
#define ANCHOR_FINALITY_KEY_LEN 32

/* Bounds of the advisory multiplier. MAX mirrors ADDRMAN_REPUTATION_MAX_MULT
 * (lib/net/include/net/addrman.h:50). It is a deliberate LOCAL copy: lib/net
 * ranks BELOW lib/policy in config/lib_module_order.def, so including
 ...
 * lib/test/src/test_anchor_finality.c, which sits above both — so this copy
 ...
 * lib/policy/src/anchor_finality.c), and a diagnostics dumper reporting an
