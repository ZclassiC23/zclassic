 ...
 * (=1), u8 kind, then the kind's body. FIND_NODE carries query_id(16) ||
 * target_node_id(32); NODES carries query_id(16) || u32 LE
 * contacts_wire_len || a complete zcode_dht_contacts.v1 blob (the sibling
 * codec in vcs/zcode_dht.h, capped at VCS_ZCODE_DHT_K contacts), with
 * contacts_wire_len == 0 meaning "no closer contacts known". */
...
#define VCS_ZCODE_DHT_FIND_NODE_WIRE_BYTES \
    (VCS_ZCODE_DHT_MSGS_HEADER_BYTES + 48u)
#define VCS_ZCODE_DHT_NODES_MAX_WIRE_BYTES \
    (VCS_ZCODE_DHT_MSGS_HEADER_BYTES + VCS_ZCODE_DHT_MSG_QUERY_ID_BYTES + 4u + \
 ...
 * reproduces the input wire byte-for-byte. query_id and (for FIND_NODE)
 * target_node_id must be non-zero; NODES contact sets go through the
 * contacts codec, so duplicate or zero contact node ids are rejected. */
 ...
 * header + body exactly (trailing bytes rejected), an all-zero query_id, an
 * all-zero FIND_NODE target, a contacts_wire_len that does not consume the
 * remaining bytes exactly, and any embedded blob the contacts codec rejects
 * (its error is propagated, so over-K contact sets fail as ERR_LIMIT). */
