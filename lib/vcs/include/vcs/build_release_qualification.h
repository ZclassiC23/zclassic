...
#define VCS_BUILD_RELEASE_CONFIRMATION_VERSION 1u
#define VCS_BUILD_RELEASE_CONFIRMATION_WIRE_BYTES 384u
#define VCS_BUILD_RELEASE_QUALIFICATION_VERSION 1u
#define VCS_BUILD_RELEASE_QUALIFICATION_WIRE_BYTES 304u
 ...
 * captured physical evidence. Software must not infer physical independence
 * from worker keys, hostnames, IP addresses, or this signature alone. */
struct vcs_build_release_confirmation_v1 {
 ...
 * receipt ledger. */
struct vcs_build_release_qualification_v1 {
    uint16_t schema_version;
    uint16_t flags;
    uint8_t action_root[32];
    uint8_t artifact_root[32];
