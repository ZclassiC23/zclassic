...
/* ── service_receipt — dual-signed verified-byte receipt codec ───────
 *
 * See vcs/service_receipt.h for the frozen 286-byte wire, the trust
 * model (advisory reputation, never consensus) and the authority
 * boundary: this is a pure codec, no store/book/network writes. It adds
 * only dependencies package_release already links (libsecp256k1,
 * SHA3-256). */
