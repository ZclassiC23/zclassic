 ...
 *
 * Tests for the writer census (app/controllers/src/fact_writers.c) — the
...
static bool fw_setup(void)
{
    ...
    if (!fw_write(FW_FIX, "lib/storage/include/storage/fw_fix_keys.h", FW_KEYS_H) ||
        !fw_write(FW_FIX, "lib/storage/src/fw_fix_a.c", FW_A_C) ||
        !fw_write(FW_FIX, "lib/storage/src/fw_fix_b.c", FW_B_C) ||
        !fw_write(FW_FIX, "lib/storage/src/fw_fix_c.c", FW_C_C) ||
        !fw_write(FW_FIX, "lib/storage/src/fw_fix_d.c", FW_D_C) ||
        !fw_write(FW_FIX, "lib/storage/src/fw_fix_e.c", FW_E_C) ||
        !fw_write(FW_FIX, "lib/storage/src/fw_fix_f.c", FW_F_C))
        return false;
