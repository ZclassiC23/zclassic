 ...
 *
 * Tests for the bg_validation_store seam.
 *
...
#define BGV_CHECK(name, expr) do {                          \
    printf("bg_validation_store_port: %s... ", (name));     \
    if ((expr)) { printf("OK\n"); }                         \
    else { printf("FAIL\n"); failures++; }                  \
} while (0)
...
int test_bg_validation_store_port(void)
{
    ...
    {
        struct node_db ndb;
        BGV_CHECK("open in-memory node db",
                  node_db_open(&ndb, ":memory:") && ndb.open);

        struct bg_validation_store_port store = {0};
        BGV_CHECK("bind store port",
                  bg_validation_store_sqlite_bind(&ndb, &store));
        ...
        int got = 0;
        BGV_CHECK("load after save returns true",
                  store.load_progress(store.self, &got));
        BGV_CHECK("loaded cursor == 1000", got == 1000);

        /* 3. overwrite. */
        BGV_CHECK("save 2000 overwrites",
                  store.save_progress(store.self, 2000));
        got = 0;
        BGV_CHECK("load reads overwritten 2000",
                  store.load_progress(store.self, &got) && got == 2000);

        /* 4. clear to the genesis-restart sentinel. */
        BGV_CHECK("save -1 clears cursor",
    ...
    {
        struct bg_validation_store_port store = {0};
        BGV_CHECK("bind rejects NULL out_port",
                  !bg_validation_store_sqlite_bind((struct node_db *)0x1,
                                                   NULL));

        /* A port bound over a NULL db degrades gracefully. */
        BGV_CHECK("bind accepts NULL db",
                  bg_validation_store_sqlite_bind(NULL, &store));
        int h = 7;
        BGV_CHECK("load over NULL db returns false",
                  !store.load_progress(store.self, &h));
