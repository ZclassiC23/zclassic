...
static void transaction_lab_p2pkh_script(struct script *script)
{
    ...
    for (size_t i = 0; i < 20; i++)
        script->data[sizeof(prefix) + i] = (uint8_t)(0x40u + i);
...
{
    if (!op_return || op_return_len == 0 || !out)
        return false;
    ...
    uint256_set_null(&coinbase_txid);
    uint256_set_null(&funding_txid);
    bool ready = simnet_mint_coinbase(&sim, &coinbase_txid) &&
        simnet_spend(&sim, &coinbase_txid, 0, TXLAB_FUND_ZAT,
                     &funding_txid);

    struct transaction tx;
    transaction_init(&tx);
    bool built = ready && transaction_lab_build_op_return(
        &tx, &funding_txid, op_return, op_return_len);
    ...
    const struct transaction_lab_simnet_receipt *receipt)
{
    if (!ndb || !ndb->open || !receipt || receipt->mined_height < 0 ||
        receipt->transaction.num_vout < 2)
        return false;
    struct block block;
    block_init(&block);
