...
static int wr_test_rescan_reports_missing_bodies(void)
{
    ...
    int found = wallet_rescan_stats(&g_wr_wallet, &chain, 0, 4,
                                    WR_DIR, &st);

    WR_CHECK("rescan over body-less heights finds nothing", found == 0);
    WR_CHECK("rescan counted a non-empty range", st.blocks_in_range >= 1);
    WR_CHECK("rescan scanned ZERO blocks (no bodies on disk)",
             st.blocks_scanned == 0);
    WR_CHECK("rescan reports every height as no_body, not as empty",
             st.blocks_no_body == st.blocks_in_range);
    WR_CHECK("rescan read no unreadable bodies", st.blocks_unreadable == 0);
    WR_CHECK("rescan says the shielded scan did not run",
             !st.shielded_scan_ran);
    WR_CHECK("rescan reports zero sapling keys", st.sapling_keys == 0);
