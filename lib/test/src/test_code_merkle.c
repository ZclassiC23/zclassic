 ...
 *   3. locality         — the change under lib/net moves lib/net's subtree root
 *                         and the tree root, and leaves lib/crypto's subtree
 *                         root byte-identical.
...
{
    bool ok = true;
    ok = ok && cm_write(dir, "lib/net/include/net/cm_a.h",
                        "#ifndef CM_A_H\n#define CM_A_H\nint cm_a(void);\n#endif\n");
    ok = ok && cm_write(dir, "lib/net/src/cm_a.c",
                        "/* cm_a — merkle fixture. */\n#include \"net/cm_a.h\"\n"
                        "int cm_a(void)\n{\n    return 1;\n}\n");
    ok = ok && cm_write(dir, "lib/net/src/cm_b.c",
                        "/* cm_b — merkle fixture. */\nint cm_b(void)\n{\n"
                        "    return 2;\n}\n");
    ok = ok && cm_write(dir, "lib/crypto/src/cm_c.c",
                        "/* cm_c — merkle fixture. */\nint cm_c(void)\n{\n"
                        "    return 3;\n}\n");
...
static int test_cm_sensitivity_and_locality(void)
{
    ...
    TEST("code_merkle: one changed byte moves one leaf + its ancestors + the "
         "root, and leaves every other leaf and lib/crypto byte-identical") {
        cm_reset();
        ASSERT(cm_fixture(CM_FIX));
        ...
        ASSERT(cm_subtree_hex(before, "lib/net", net_before));
        ASSERT(cm_subtree_hex(before, "lib/crypto", crypto_before));
        ASSERT(cm_leaf_hex(before, "lib/net/src/cm_a.c", a_before));
        ASSERT(cm_leaf_hex(before, "lib/net/src/cm_b.c", b_before));
        ASSERT(cm_leaf_hex(before, "lib/crypto/src/cm_c.c", c_before));
        ...
        ASSERT(cm_write(CM_FIX, "lib/net/src/cm_a.c",
                        "/* cm_a — merkle fixture. */\n#include \"net/cm_a.h\"\n"
                        "int cm_a(void)\n{\n    return 9;\n}\n"));
        ...
        ASSERT(cm_subtree_hex(after, "lib/net", net_after));
        ASSERT(cm_subtree_hex(after, "lib/crypto", crypto_after));
        ASSERT(cm_leaf_hex(after, "lib/net/src/cm_a.c", a_after));
        ASSERT(cm_leaf_hex(after, "lib/net/src/cm_b.c", b_after));
...
static int test_cm_incremental(void)
{
    ...
    TEST("code_merkle: after editing one file a warm refresh re-reads 1 of 5 "
        ...
        /* "", core, lib, lib/crypto, lib/crypto/src, lib/net,
         * lib/net/include, lib/net/include/net, lib/net/src */
        ...
        /* now edit one file. */
        ASSERT(cm_write(CM_FIX, "lib/net/src/cm_b.c",
...
static int test_cm_command(void)
{
    ...
    TEST("code.merkle: tree / directory / file / absent answers survive the "
        ...
        /* the fixture's top level is exactly core/ and lib/ */
        ASSERT(strstr(buf, "\"children_total\":2") != NULL);
        ASSERT(strstr(buf, "\"children_truncated\":false") != NULL);
        ASSERT(strstr(buf, "\"path\":\"core\"") != NULL);
        ASSERT(strstr(buf, "\"path\":\"lib\"") != NULL);
        ...
        char net_hex[65];
        ASSERT(cm_subtree_hex(ref, "lib/net", net_hex));
        ASSERT(strcmp(net_hex, root_hex) != 0);
        cm_call("lib/net", &reply);
        ASSERT(reply.status != ZCL_COMMAND_STATUS_FAILED);
        n = cm_render(&reply, buf, sizeof(buf));
        ASSERT(n > 0 && n <= ZCL_COMMAND_LIST_BUDGET);
        ASSERT(strstr(buf, "\"kind\":\"dir\"") != NULL);
        ASSERT(strstr(buf, "\"path\":\"lib/net\"") != NULL);
        snprintf(want, sizeof(want), "\"digest\":\"%s\"", net_hex);
        ASSERT(strstr(buf, want) != NULL);
        snprintf(want, sizeof(want), "\"tree_root\":\"%s\"", root_hex);
        ASSERT(strstr(buf, want) != NULL);
        ASSERT(strstr(buf, "\"file_count\":3") != NULL);
        ASSERT(strstr(buf, "\"path\":\"lib/net/include\"") != NULL);
        ASSERT(strstr(buf, "\"path\":\"lib/net/src\"") != NULL);
        zcl_command_reply_free(&reply);

        /* (c) one file leaf */
        char leaf_hex[65];
        ASSERT(cm_leaf_hex(ref, "lib/net/src/cm_b.c", leaf_hex));
        cm_call("lib/net/src/cm_b.c", &reply);
        ASSERT(reply.status != ZCL_COMMAND_STATUS_FAILED);
        n = cm_render(&reply, buf, sizeof(buf));
        ASSERT(n > 0 && n <= ZCL_COMMAND_LIST_BUDGET);
        ...
        cm_call("lib/net/src/not_here.c", &reply);
        ASSERT(reply.status != ZCL_COMMAND_STATUS_FAILED);
        n = cm_render(&reply, buf, sizeof(buf));
        ASSERT(n > 0 && n <= ZCL_COMMAND_LIST_BUDGET);
