 ...
 * (app/controllers/include/controllers/operator_needed_policy.def).
 *
 * WHY THIS EXISTS: the public REST status endpoint
 * (app/controllers/src/api_controller_status.c) and the agent first-call
 * summary (app/controllers/src/event_agent_summary.c) each used to carry their
