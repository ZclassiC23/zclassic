...
#define TEST_LIST(X) \
    ...
    X(bn254) X(merkle_tree) X(merkle_malleability) X(slp) X(models) X(core) X(overlay) X(znam) X(znam_site) X(zanc) X(zid) X(zid_identity) X(zid_seniority) X(zdir) X(identity_command) X(zdesc) X(zendp) X(proof_chain) X(htlc) \
    X(swap_settlement) \
