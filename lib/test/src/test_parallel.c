...
#define TEST_LIST(X) \
    ...
    X(long_op) \
    X(db_txn) X(sync_service) X(node_db_catchup_service) X(utxo_mirror_sync) X(snapshot_sync_service) \
    ...
    X(clock) X(rng) X(seed_tape) X(postmortem) X(simnet) X(simnet_cluster) X(simnet_wire) X(simnet_wire_ibd) X(simnet_byzantine) X(simnet_txkit) X(simnet_contract) X(simnet_doublespend) X(simnet_chained_tx) X(simnet_block_sigops) X(simnet_duplicate_input) X(simnet_value_inflation) X(simnet_fee_range) X(simnet_empty_vin_vout) X(simnet_input_value_range) X(simnet_sapling_activation) X(simnet_sapling_shielded_send) X(simnet_zmsg_onchain) X(simnet_fuzz) X(util_signal_handler) X(chaos_harness) X(stage) X(stage_anchor) X(mailbox) X(mailbox_adoption) \
    ...
    X(zslp_store_port) \
    X(sapling_tree) X(sapling_ckpt_persist) X(heartbeat) X(syncdiag_rpc) X(peer_lifecycle) \
    ...
    X(rpc_safety) X(app_context) X(service_kernel) X(thread_registry) X(sync_state_fsm) \
