...
#define TEST_LIST(X) \
    ...
    X(explorer) X(explorer_rpc_call) X(explorer_index) X(format_helpers_codec) X(mining) X(utxo_commitment) X(mmr) X(mmb) X(sha3_windows) X(keccak_avx512) X(sha3_256_x4) X(sha256_isa_parity) \
