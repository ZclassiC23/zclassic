...
#define TEST_LIST(X) \
    ...
    X(seal_kv) X(sha3_sidecar_io) X(seal_ratify) X(vcs_core) X(vcs_release) X(vcs_accept) X(zcode_store) X(zcode_publish) X(zcode_recipe) X(zcode_contributor) X(vcs_devloop) X(codeindex) X(testcache) \
