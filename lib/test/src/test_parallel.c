...
#define TEST_LIST(X) \
    ...
    X(bn254) X(merkle_tree) X(merkle_malleability) X(slp) X(models) X(core) X(znam) X(znam_site) X(zanc) X(htlc) \
    X(swap_settlement) \
    ...
    X(lag_slo) X(boot_phase) X(boot_status) X(path_check) X(parse_num) X(boot_progress) X(supervisor) \
    ...
    X(rpc_safety) X(app_context) X(service_kernel) X(thread_registry) X(self_backtrace) X(sync_state_fsm) \
