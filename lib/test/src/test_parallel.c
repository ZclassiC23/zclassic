...
#define TEST_LIST(X) \
    ...
    X(bn254) X(merkle_tree) X(merkle_malleability) X(slp) X(models) X(core) X(overlay) X(overlay_parse_parity) X(znam) X(znam_site) X(zanc) X(zid) X(zid_identity) X(zid_seniority) X(zid_seniority_binding) X(zdir) X(zdir_write_path) X(identity_command) X(zdesc) X(zendp) X(zendp_records) X(proof_chain) X(htlc) \
    X(swap_settlement) \
    ...
    X(chain_tip_watchdog_bounded_restart) X(blocker) X(blocker_handoff) X(blocker_remedy_dispatch) X(cpu_topology) X(hw_profile) X(hw_bench) X(log_level) X(service_state) \
    ...
    X(zslp_store_port) \
    X(sapling_tree) X(sapling_ckpt_persist) X(shutdown_marker) X(shutdown_stagewatch) X(cold_start_driver) X(heartbeat) X(syncdiag_rpc) X(peer_lifecycle) X(telemetry_ontology) \
