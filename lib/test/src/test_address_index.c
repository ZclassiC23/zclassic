...
/* Holds the progress-store lock on a NON-test thread so the recursive mutex
 * cannot let the test's own trylock succeed. Announces the acquisition, then
...
int test_address_index(void)
{
     ...
     * a snapshot-seed floor — both surface a NAMED typed blocker. */
    {
         ...
         * took a BLOCKING progress-store lock it sat behind the reducer
         ...
         * 120 s limit — seven kills in forty minutes on a node that was
         * folding blocks correctly the whole time.
         *
         * The lock is RECURSIVE, so holding it on THIS thread would let
         * trylock succeed and prove nothing. It must be held by another
         * thread. Completing at all proves non-blocking (a blocking acquire
         * would park here until the helper releases); the yield counter
         * proves the trylock path is the reason, with no timing assertion. */
        {
            ...
            if (rc == 0) {
                 ...
                 * raises the blocker, proving the yield was the lock and not a
                 * broken read path. */
                index_fold_note_absent_body("address_index", "address_index",
                                            db, 500);
