...
bool connman_request_v2_upgrade(struct connman *cm, struct p2p_node *node)
{
     ...
     * Its first connection may still be legacy because v2 capability was not
     * known until VERSION arrived. Preserve exactly that same signed endpoint
     * for the one controlled Noise reconnect. This is also required in
     * -connect mode, where addrman is intentionally not a dial source. */
