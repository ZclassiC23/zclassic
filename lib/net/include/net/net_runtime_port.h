 ...
 *
 * net_runtime_port — the one seam through which lib/net reaches the
 * composition root.
 *
 * config/ wires the process together and therefore sits ABOVE lib/. Before
 * this port existed, three lib/net translation units called straight into
 * config/ (db_service_node_db, boot_snapshot_offer_state_is_sovereign,
 * boot_snapshot_offer_artifact_is_eligible), which made lib/net and config/
 * mutually dependent: config/ composes the message processor, and the
 * message processor called back into config/ by name.
 *
 * The direction is now one-way. lib/net declares what it needs; config/
 * registers the implementations at process start. Nothing in lib/net names
 * a config/ symbol.
 *
 * FAIL-CLOSED: with no port registered every accessor returns NULL/false.
 * A binary that links lib/net without the composition root (a fuzz target,
