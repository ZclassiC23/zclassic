 ...
 *
 * directory_influence_port — the seam through which lib/net asks whether the
 * peer/name DIRECTORY is currently allowed to influence anything.
 *
 * The policy itself lives upstairs
 * (app/services/include/services/directory_influence_policy.h) because it
 * reads the netsplit detector and the sync capability table, both of which
 * sit above lib/. lib/net declares what it needs; the composition root
 * registers the implementation at process start. Same one-way shape as
 * net/net_runtime_port.h — nothing in lib/net names an app/ symbol.
 *
 ...
 * lib/net without the composition root — fuzz targets, standalone tools,
