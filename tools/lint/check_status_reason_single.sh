...
scan_counts() {
    awk -v amin="$LADDER_ASSIGN_MIN" -v emin="$LADDER_ELSEIF_MIN" '
        ...
        infunc {
            ...
            # A later plain assignment to one of those locals.
...
FIXTURE
    ...
    echo "[$GATE] SELFTEST PASS (clean passes; named and renamed ladders fail; a vocabulary-only ladder fails; an innocuous multi-branch flag and a single-branch flag do not count; both consolidated surfaces are clean)"
    exit 0
fi
