                               ...
                               const char *scope_name)
{
    ...
    for (size_t i = 0; i < sf->n; i++) {
        ...
        size_t len = 0;
        uint64_t declared = 0;
        if (!file_load(root, path, &bytes, &len, &declared)) {
 ...
 * directory prefixes, else the repo-root LICENSE. */
 ...
 * root), else the repo Makefile (the declared core-scope build recipe). */
                                   ...
                                   uint8_t *wire_out, size_t *wire_len_out)
{
    ...
    const char *author = author_string(def->kind);
...
int main(int argc, char **argv)
{
    ...
    for (size_t s = 0; s < scope_count; s++) {
        ...
        if (!scope_release_wire(args.repo, &files[s], &wire,
                                &run->measure, defs[s].name))
            return 1;
        if (!evidence_root(k_domain_release, wire.p, wire.len,
                           run->measure.release_root)) {
            buf_free(&wire);
            return 1;
        }
        buf_free(&wire);
        if (!scope_license_bind(args.repo, &defs[s], &run->measure))
            return 1;
        if (!scope_recipe_bind(args.repo, &defs[s], &run->measure))
            return 1;
        run->in_census = true;
    }

    /* REPRODUCIBLE: dual-materialization rederivation at HEAD. */
    bool *repro_matched =
    ...
    LOG_INFO(CENSUS_LOG, "reproduction binding method: %s", repro_method);
    ...
    for (size_t s = 0; s < scope_count; s++) {
        ...
        if (run->reproduced) {
            struct buf rwire = {0};
            if (!buf_put(&rwire, m->release_root, 32) ||
                !buf_put(&rwire, repro_method, strlen(repro_method)) ||
                !evidence_root(k_domain_reproduction, rwire.p, rwire.len,
        ...
        if (args.quality_attested && root_nonzero(run->quality_root))
        ...
        for (size_t i = 0; i < files[s].n; i++) {
            ...
            if (!file_load(args.repo, files[s].paths[i],
