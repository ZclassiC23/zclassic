...
#       persistence_adapters. This gate measures the real counts from the code
#       and FAILS if the declared counts disagree. To fix a real drift: update
#       the canonical block (and any prose) to match the code — never the other
#       way around; the code is authoritative for what exists.
#
...
echo "code-measured: test_groups=$code_test_groups port_interfaces=$code_ports persistence_adapters=$code_adapters"
...
if [ -z "${block//[[:space:]]/}" ]; then
    ...
    echo "      Add test_groups / port_interfaces / persistence_adapters declarations"
...
echo "check_doc_counts: clean — test_groups=$code_test_groups port_interfaces=$code_ports persistence_adapters=$code_adapters, docs agree and no stale phrasings found"
exit 0
