...
# purpose: Permanent sensitivity proof for the zcode package-view contract.

set -euo pipefail

ROOT="${ZCL_SOURCE_ROOT:-$(pwd -P)}"
BIN="${ZCL_DEV_BIN:-$ROOT/build/bin/zclassic23-dev}"
SOURCE="$ROOT/tools/command/native_zcode_package_view_command.c"
UNRELATED="$ROOT/app/services/src/market_moderation_service.c"
OUTPUT="${ZCL_REFLEX_ZCODE_PACKAGE_VIEW_ACCEPTANCE_OUTPUT:-$ROOT/build/dev-loop/reflex-hotfork-zcode-package-view-acceptance.json}"
STORY='zcode-package-view-contract.v1'

fail() { printf 'reflex-hotfork-zcode-package-view-acceptance: %s\n' "$*" >&2; exit 2; }
[[ -x "$BIN" ]] || fail "missing dev binary: $BIN"
[[ -f "$SOURCE" && ! -L "$SOURCE" ]] || fail "owner source is not a regular file: $SOURCE"
command -v jq >/dev/null || fail 'jq is required'

backup="$(mktemp "${TMPDIR:-/tmp}/zcl-reflex-zcode-package-view.XXXXXX")"
unrelated_backup="$(mktemp "${TMPDIR:-/tmp}/zcl-reflex-zcode-package-view-unrelated.XXXXXX")"
watcher_id=0
cleanup()
{
...
green="$(mktemp "$ROOT/tools/command/.reflex-zcode-package-view-green.XXXXXX")"
cp -p -- "$backup" "$green"
printf '\n/* ZCL_REFLEX_ZCODE_PACKAGE_VIEW_ACCEPTANCE:%s */\n' "$nonce" >>"$green"
drive_candidate "$green" STORY_GREEN
green_result="$result"

# An unrelated source edit must neither claim this story nor change its exact
# candidate roots/cache identity.
printf '\n/* ZCL_REFLEX_ZCODE_PACKAGE_VIEW_UNRELATED:%s */\n' "$nonce" >>"$UNRELATED"
unrelated_wait=true
for ((loops = 0; loops < 16; loops++)); do
...
unrelated_proof="$(mktemp "$ROOT/tools/command/.reflex-zcode-package-view-unrelated-proof.XXXXXX")"
...
mutant="$(mktemp "$ROOT/tools/command/.reflex-zcode-package-view-red.XXXXXX")"
cp -p -- "$SOURCE" "$mutant"
perl -0pi -e 's/plan\.ready_to_commit \|\| strcmp\(plan\.readiness, "blocked"\)/!plan.ready_to_commit || strcmp(plan.readiness, "blocked")/' "$mutant"
[[ "$(LC_ALL=C grep -c '!plan.ready_to_commit || strcmp(plan.readiness, "blocked")' "$mutant")" -eq 1 ]] ||
    fail 'compile-valid semantic mutation was not staged'
drive_candidate "$mutant" STORY_RED
red_result="$result"

revert="$(mktemp "$ROOT/tools/command/.reflex-zcode-package-view-revert.XXXXXX")"
cp -p -- "$backup" "$revert"
printf '\n/* ZCL_REFLEX_ZCODE_PACKAGE_VIEW_ACCEPTANCE:%s */\n' "$nonce" >>"$revert"
...
jq -n --arg owner 'tools/command/native_zcode_package_view_command.c' \
...
cp -p -- "$backup" "$SOURCE"
cp -p -- "$unrelated_backup" "$UNRELATED"
printf 'reflex-hotfork-zcode-package-view-acceptance: PASS receipt=%s\n' "$OUTPUT"
