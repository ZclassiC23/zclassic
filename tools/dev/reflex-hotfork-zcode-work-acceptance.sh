...
# purpose: Permanent sensitivity proof for the zcode work-input core.
...
fail() { printf 'reflex-hotfork-zcode-work-acceptance: %s\n' "$*" >&2; exit 2; }
[[ -x "$BIN" ]] || fail "missing dev binary: $BIN"
[[ -f "$SOURCE" && ! -L "$SOURCE" ]] || fail "owner source is not a regular file: $SOURCE"
command -v jq >/dev/null || fail 'jq is required'

backup="$(mktemp "${TMPDIR:-/tmp}/zcl-reflex-zcode-work.XXXXXX")"
unrelated_backup="$(mktemp "${TMPDIR:-/tmp}/zcl-reflex-zcode-work-unrelated.XXXXXX")"
watcher_id=0
cleanup()
{
...
green="$(mktemp "$ROOT/tools/command/.reflex-zcode-work-green.XXXXXX")"
cp -p -- "$backup" "$green"
printf '\n/* ZCL_REFLEX_ZCODE_WORK_ACCEPTANCE:%s */\n' "$nonce" >>"$green"
drive_candidate "$green" STORY_GREEN
green_result="$result"

printf '\n/* ZCL_REFLEX_ZCODE_WORK_UNRELATED:%s */\n' "$nonce" >>"$UNRELATED"
unrelated_wait=true
for ((loops = 0; loops < 16; loops++)); do
...
unrelated_proof="$(mktemp "$ROOT/tools/command/.reflex-zcode-work-unrelated-proof.XXXXXX")"
...
mutant="$(mktemp "$ROOT/tools/command/.reflex-zcode-work-red.XXXXXX")"
cp -p -- "$SOURCE" "$mutant"
perl -0pi -e 's/memcmp\(at, part, len\) == 0/memcmp(at, part, len) != 0/' "$mutant"
[[ "$(LC_ALL=C grep -c 'memcmp(at, part, len) != 0' "$mutant")" -eq 1 ]] ||
    fail 'compile-valid semantic mutation was not staged'
drive_candidate "$mutant" STORY_RED
red_result="$result"

revert="$(mktemp "$ROOT/tools/command/.reflex-zcode-work-revert.XXXXXX")"
cp -p -- "$backup" "$revert"
printf '\n/* ZCL_REFLEX_ZCODE_WORK_ACCEPTANCE:%s */\n' "$nonce" >>"$revert"
...
jq -n --arg owner 'tools/command/native_zcode_work_command.c' \
...
cp -p -- "$backup" "$SOURCE"
cp -p -- "$unrelated_backup" "$UNRELATED"
printf 'reflex-hotfork-zcode-work-acceptance: PASS receipt=%s\n' "$OUTPUT"
