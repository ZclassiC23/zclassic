...
# purpose: Permanent sensitivity proof for the buyer-want view contract.
...
fail() { printf 'reflex-hotfork-shop-want-view-acceptance: %s\n' "$*" >&2; exit 2; }
...
# Prove a real edit outside the capsule neither steals this owner story nor
# enters its object/module cache identity.
printf '\n/* ZCL_REFLEX_SHOP_WANT_VIEW_UNRELATED:%s */\n' "$nonce" >>"$UNRELATED"
unrelated_wait=true
for ((loops = 0; loops < 16; loops++)); do
