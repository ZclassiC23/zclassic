    ...
    struct node_db *ndb, const struct db_build_receipt *receipt, int64_t now)
{
    ...
    if (strcmp(action.kind, VCS_BUILD_ACTION_KIND_V1) == 0 &&
        !bf_lower_hex_id(receipt->observation_sha3))
...
static struct zcl_result bf_observation_verify(
    ...
    int64_t now)
{
    ...
    struct db_build_action action;
    struct db_build_job job;
    if (!ndb || !ndb->open || !bf_lower_hex_id(receipt_id) ||
...
static bool bf_preserved_receipt_signature_valid(
    ...
    struct build_fabric_shadow_match *out)
{
    ...
    if (!ndb || !ndb->open || !workspace || !out ||
        !bf_lower_hex_id(primary_receipt_id) ||
        !bf_lower_hex_id(shadow_receipt_id) ||
    ...
    if (!bf_preserved_receipt_signature_valid(ndb, &primary) ||
        !bf_preserved_receipt_signature_valid(ndb, &shadow))
    ...
    struct zcl_result bound = bf_observation_verify(
        workspace, &job, &action, &primary);
    if (!bound.ok) return bound;
    bound = bf_observation_verify(workspace, &job, &action, &shadow);
    if (!bound.ok) return bound;
    struct vcs_build_execution_observation_v1 a, b;
    struct zcl_result loaded = bf_observation_load(
        workspace, primary.observation_sha3, &a);
    if (!loaded.ok) return loaded;
    loaded = bf_observation_load(workspace, shadow.observation_sha3, &b);
    if (!loaded.ok) return loaded;
