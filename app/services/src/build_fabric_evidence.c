    ...
    char receipt_id[BUILD_FABRIC_ID_HEX + 1])
{
    ...
    if (db_build_worker_find(ndb, worker_id, &worker)) {
        ...
        (void)snprintf(worker.capabilities, sizeof(worker.capabilities),
                       "p2p-observed,c23.compile.preprocessed.v1");
        worker.last_seen_at = now;
    }
    ...
    int64_t now, struct build_fabric_proof_evaluation *out)
{
    ...
    int row_count = db_build_job_receipts(
        ndb, action.job_id, rows, VCS_ZCODE_PROOF_SET_MAX_RECEIPTS);
    ...
    for (int i = 0; i < row_count; i++) {
        if (strcmp(rows[i].action_id, action_id) != 0 ||
            !rows[i].work_receipt_sha3[0])
            continue;
        ...
        verified = verified && zcl_hex_decode_lower(
            action.action_id, action_root, 32) &&
    ...
    if (selected == SIZE_MAX) return ZCL_OK;
    ...
    for (size_t i = 0; i < valid_count; i++) {
        if (memcmp(valid[i].receipt.output_root, selected_output, 32) != 0)
            continue;
        if (valid[i].local) have_local = true; else have_remote = true;
        if (valid[i].approved) {
            bool duplicate = false;
    ...
    out->matching_receipts = distinct_count;
    out->local_reproduced = have_local && have_remote;
    size_t needed = policy.minimum_compile_receipts;
    if (policy.minimum_matching_receipts > needed)
        needed = policy.minimum_matching_receipts;
    out->quorum_satisfied = distinct_count >= needed;
    out->compile_satisfied = out->local_reproduced || out->quorum_satisfied;
    uint32_t unsupported = policy.required_proofs &
        ~(VCS_ZCODE_PROOF_COMPILE | VCS_ZCODE_PROOF_LOCAL_REPRODUCTION);
    bool local_required = (policy.required_proofs &
                           VCS_ZCODE_PROOF_LOCAL_REPRODUCTION) != 0;
    out->policy_satisfied = out->compile_satisfied && unsupported == 0 &&
        (!local_required || out->local_reproduced);
    zcl_hex_encode(selected_output, 32, out->output_root_sha3);
    for (size_t i = 0; i < valid_count; i++) {
        bool matching = memcmp(valid[i].receipt.output_root,
                               selected_output, 32) == 0;
        bool contributes = matching &&
            (out->local_reproduced ||
             (out->quorum_satisfied && valid[i].approved));
    ...
    for (size_t i = 0; i < valid_count && saved; i++) {
        if (strcmp(valid[i].row.trust_state, "REMOTE_OBSERVED") != 0 ||
            ...
            continue;
        if (out->local_reproduced)
            (void)snprintf(valid[i].row.trust_state,
                           sizeof(valid[i].row.trust_state),
                           "LOCAL_REPRODUCED");
        else if (out->quorum_satisfied && valid[i].approved)
            (void)snprintf(valid[i].row.trust_state,
                           sizeof(valid[i].row.trust_state),
                           "QUORUM_MATCHED");
        else
