    ...
    uint32_t *model_len)
{
    ...
    *model_len = zcl_read_u32_le(in + 8u);
    return (*flags & ~UI_HOST_FLAG_WAIT_EVENT) == 0 &&
...
static void ui_host_window_ready(void *context)
{
    ...
    ready->sent = ui_host_send_all(ready->fd, reply, sizeof(reply));
}
...
static int ui_host_worker(int listener, int client)
{
    ...
    char why[192];
    if (!zcl_present_model_decode_v1(wire, model_len, &model,
                                     why, sizeof(why)) ||
        !zcl_present_model_render_v1(&model, &bitmap, why, sizeof(why))) {
        ui_host_send_rejected(client, UI_HOST_PHASE_READY);
        close(client);
        return 2;
    }
    ...
    bool shown = zcl_present_window_run_actions_v1(
        &window, model.action_count, ui_host_window_ready, &ready,
