 ...
 * app/views/src/zcode_view{,_pages}.c; this file owns routing, the
 * projection reads, and the CAS download path.
 *
 * Every read goes through the SAME lib/vcs projections the zcode.* typed
                                 ...
                                 const char *datadir)
{
    ...
    if (zs_path_eq(route, "/zcode/packages") ||
        zs_path_eq(route, "/zcode/packages/"))
