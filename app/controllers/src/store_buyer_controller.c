 ...
 * app/services/src/store_buyer.c. This file parses params, calls one service
...
static bool sbc_refuse(struct json_value *result, enum store_buyer_status st,
                       const char *detail)
{
                                 ...
                                 struct json_value *result)
{
    ...
    if (st != STORE_BUYER_OK)
        return sbc_refuse(result, st, NULL);
                             ...
                             struct json_value *result)
{
    ...
    if (st != STORE_BUYER_OK) {
        (void)store_buyer_fail(datadir, purchase_id, st, NULL);
        return sbc_refuse(result, st, NULL);
    }
    ...
    if (!sent) {
        ...
        (void)store_buyer_fail(datadir, purchase_id, mapped, why);
        bool r = sbc_refuse(result, mapped, why);
    ...
    (void)json_push_kv_bool(result, "recorded", rec == STORE_BUYER_OK);
    json_free(&send_result);
    return true;
}
