 ...
 * ~3 blk/s measured in docs/work/never-stuck-roadmap-2026-06-28.md. */
...
static void diagnostics_catalog_push_entry(struct json_value *arr,
                                           const struct dump_entry *e)
{
                           ...
                           struct json_value *result)
{
    ...
    RPC_HELP(help, result,
        ...
        "state_class, owner_shape, freshness, cost, accepts_key, ... }] }");
    ...
    json_push_kv_int(result, "count",
                     (int64_t)(sizeof(g_dumpers) / sizeof(g_dumpers[0])));

    json_init(&subsystems);
    json_set_array(&subsystems);
    for (size_t i = 0; i < sizeof(g_dumpers) / sizeof(g_dumpers[0]); i++)
        diagnostics_catalog_push_entry(&subsystems, &g_dumpers[i]);
