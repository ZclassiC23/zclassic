...
/* core.status.brief: a compact FLAT projection of exactly the sync/serving
 * fields an operator or AI checks on every call, so a token-limited caller
 * never has to pipe the ~55KB core.status body through grep/tr. Every key is
 * a real field the full status composition already produces
 * (served height, validated header tip, sync gap, sync-state-machine state,
 * healthcheck healthy/serving/tip_advance_age_seconds, peer count, and the
 * dominant typed blocker id) — this handler just fetches the same six RPCs
 * and keeps only the scalar answer, never a container. Unknown fields are
 * `null`, never a fabricated zero, matching status_push_int_if_known /
 * status_push_bool_if_known elsewhere in this file. */
                                   ...
                                   struct zcl_native_body_err *err)
{
    ...
    json_init(&root);
    json_set_object(&root);
    status_push_int_if_known(&root, "served_height", height_ok,
                             served_height);
    status_push_int_if_known(&root, "header_height", header_ok,
                             header_height);
    ...
    json_free(&chain_j);
    json_free(&height_j);
    free(h); free(ci); free(s); free(hc); free(p); free(bl);
    if (!out) {
        err->status = ZCL_NATIVE_BODY_INTERNAL;
        snprintf(err->message, sizeof(err->message),
