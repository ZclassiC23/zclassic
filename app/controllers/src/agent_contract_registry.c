...
static const struct agent_contract_command_surface g_agent_command_surfaces[] = {
    ...
    CONTRACT_COMMAND("agentmap.commands.core", 9, "lanes", "agentlanes",
      "canonical/soak/dev topology and restart/deploy rules"),
    CONTRACT_COMMAND("agentmap.commands.core", 10, "liveness", "agentliveness",
      "lane/service/supervisor/background-quality liveness"),
    CONTRACT_COMMAND("agentmap.commands.core", 11, "deploy_guard", "agentdeployguard",
      "C-native deploy/restart allow-refuse decision"),
    DIRECT_COMMAND("agentmap.commands.core", 12, "command_center",
      "zclassic23 agent", "zcl_operator_summary",
      "one-shot live node status, blockers, next action, and drill-down tools"),
    DIRECT_COMMAND("agentmap.commands.core", 13, "background_quality",
    ...
    CONTRACT_COMMAND("agentmap.telemetry", 9, "node_db", "dbquery",
      "SELECT-only node.db inspection with limits"),
    CONTRACT_COMMAND("agentmap.telemetry", 10, "events", "eventlog",
      "recent structured node events"),
    DIRECT_COMMAND("agentmap.telemetry", 11, "quality_lanes",
      "make quality-linger-status", "zcl_agent_build",
...
static const struct agent_contract_field_surface g_agent_field_surfaces[] = {
    ...
    FIELD_BINDING("agentops.first_call", 12, "dev_status_command",
                  "dev_status_tool", "agentdevstatus"),
    FIELD_BINDING("agentops.first_call", 13, "deploy_guard_command",
