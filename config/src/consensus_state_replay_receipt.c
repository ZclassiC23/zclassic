...
static bool rr_read_file(const char *datadir, struct rr_receipt *r)
{
    char path[PATH_MAX];
    if (!rr_receipt_path(datadir, path, sizeof(path)))
        return false;
...
{
    if (!manifest || !bundle_file_digest || !datadir || !datadir[0])
        return false;

    struct rr_receipt r;
    memset(&r, 0, sizeof(r));
    /* Reads the keyed file AND re-verifies its self-binding receipt_digest. */
    if (!rr_read_file(datadir, &r)) {
